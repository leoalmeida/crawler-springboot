...
public class CrawlerHandler {
    ...
    private static final Pattern FILTERS =
            Pattern.compile(".*(\\.(css|js|gif|jpg" + "|png|mp3|mp4|zip|gz|pdf|xls|xlsx|doc|docx))$");
    ...
    private Crawler findKeywordInText(final String url, final String text, final Crawler processEntity) {
    ...
    private void addInternalUnvisitedUrls(
            final Document doc, final String baseLink, final Set<String> visitedUrls, final Queue<String> urlsToVisit) {
    ...
    private Crawler processCurrentUrl(
            final String currentUrl,
            final String baseLink,
            final Crawler currentRequestState,
    ...
    public Crawler crawlResource(final String link, final Crawler request) {
        ...
        while (!urlsToVisit.isEmpty() && visitedUrls.size() <= MAX_VISITED_URLS) {
            ...
            try {
                currentRequestState =
                        processCurrentUrl(currentUrl, link, currentRequestState, visitedUrls, urlsToVisit);
            } catch (IOException e) {
                LOGGER.error(MessageFormat.format(
                        "IOException Error while crawling [{0}]: [{1}]", currentUrl, e.getMessage()));
            }
        }
